...
var Crawler = function () {
  ...
  _createClass(Crawler, [{
    ...
    key: 'snap',
    value: async function snap() {
      var _this = this;

      while (this.paths.length) {
        var urlPath = this.paths.unshift();
        ...
        try {
          var window = await (0, _snapshot2.default)(this.protocol, this.host, urlPath, this.snapshotDelay);
          ...
          this.handler({ urlPath: urlPath, html: html });
    ...
    value: function extractNewLinks(window, currentPath) {
      ...
      Object.keys(tagAttributeMap).forEach(function (tagName) {
        ...
        Array.from(document.querySelectorAll(tagName + '[' + urlAttribute + ']')).forEach(function (element) {
          ...
          if (_this2.processed.has(relativePath)) return;
